...
public class Base64Util {
	...
	static InputStream encode(InputStream inputStream) {
		try {
			int bytesToRead = inputStream.available(); // 对文件流可靠, 对网络流不可�?
	 ...
	 * @param charsetName
	 *            字符编码
	 * @return 是BASE64编码返回true，否则返回false
	 * @creator chengking
	 */
	public static boolean isBase64Encoded(String strEncoded, String charsetName) {
		...
		byte[] base64Data;
		if (false == StringHelper.isEmpty(charsetName)) {
			try {
		...
		if (false == Base64.isArrayByteBase64(base64Data)) {
			return false;
		}
		try {
			if (strEncoded.equals(encode(decode(strEncoded)))) {
				return true;
			}
		} catch (Throwable e) {
			e.printStackTrace();
	...
	public static boolean isBase64Encoded(String strEncoded) {
		return isBase64Encoded(strEncoded, "");
	}
}
...
class Base64 {
	 ...
	 * @param arrayOctect
	 *            byte array to test
	 * @return true if all bytes are valid characters in the Base64 alphabet or if the byte array is empty; false,
	 *         otherwise
	 */
