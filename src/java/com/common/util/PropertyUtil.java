...
public class PropertyUtil {
	 ...
	 * @deprecated ChengKing: 异常处理不恰当，不应被吞没！使用 {@link #assertAndLoadProperties(File)}代替.
	 */
	 ...
	 * @creator ChengKing
	 */
	public static Properties assertAndLoadProperties(File f) throws Exception {
		if (f == null || false == f.exists()) {
		...
		try {
			...
			LOG.error("erron on load file: " + f, e);
			throw new Exception("fail to read [" + f + "]", e);
		} finally {
	 ...
	 * @deprecated ChengKing 请使用{@link #assertAndLoadFromResource(String)}代替.
	 */
	 ...
	 *            给定的资源文件
	 * @return 给定的资源文件中获取到的属性. 如果有异常发生, 则返回一个empty的属性.
	 * @deprecated ChengKing 请使用 {@link #assertAndLoadFromResource(Class, String)}代替.
	 */
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static Properties assertAndLoadFromResource(Class clazz, String resName) throws Exception,
	Exception {
		if (resName == null) {
		...
		if (resUrl == null) {
			if (resName.startsWith("/")) {
				throw new Exception("resource [ " + resName + " ] not found in [ " + clazz.getResource("/")
						+ " ]!");
			} else {
				throw new Exception("resource [ " + resName + " ] not found in [ " + clazz.getResource("/")
		...
		try {
			...
			throw new Exception("fail to open [" + resUrl + "]", e);
		}
		...
		try {
			...
			throw new Exception("fail to read [" + resUrl + "]", e);
		} finally {
			CloseUtil.closeInputStream(fis);
		}
		return props;
