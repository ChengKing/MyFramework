...
public class UrlUtil {
	...
	/**
	 * 对给定的字符串进行URL解码；如果解码失败，返回原串. From JDK Javadoc: Decodes a
	...
	public static String replaceHostInUrl(String strUrl, String host) {
		...
		try {
			...
			throw new RuntimeException("MalformedURL: [" + strUrl + "]", e);
		}
		try {
			URL replaced = new URL(url.getProtocol(), host, url.getPort(), url.getFile());
			return replaced.toString();
		} catch (MalformedURLException e) {
			throw new RuntimeException("MalformedURL because host: [" + host + "]", e);
		}
	}
